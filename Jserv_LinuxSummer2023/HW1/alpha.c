 ...
 *
 * AVL-trees可以提供接近最優解的二元樹布局，但會消耗許多記憶體資源。紅黑樹的二元樹布局會略
 * 遜於AVL但比較容易操作。而S樹則在搜尋過程中來平衡樹的結構來實現快速插入與刪除。
 *
 ...
 *
 * S樹依靠四種二元搜索樹的基本操作: 左旋轉, 右旋轉,
 * 左替換與右替換。左替換與右替換為實作傳統
 * 二元樹時使用，兩種方法各別辨識左右測子樹上的下以個節點並將準備刪除的點替換成被辨識到的點。
 *
 ...
 *
 * 相比之下，rotate_left與rotate_right兩個函式專門用於更新階段，用於重新平衡樹結構。目前的
 * 實作中於插入與移除階段皆會使用。但從理論來說可以進行插入,
 * 移除需住查找與更新操作任意位置。需住
 * 意更新的頻率會直接影響二元樹結構最佳化的程度。然而需注意每次進行一次更新都會收到一定的時間損失。
 *
 ...
 *
 * 更新相關函式相對直覺:
 * 當一個特定節點向左或向右的程度越過閥值時將對該節點進行右旋或左旋，同時
 * 節點仍會不斷進行更新。若更新過程中節點的hint變成0或是之前的狀態，則會直接對節點的parent進行
 * 修改，修改後的節點在更新之前就已經存在。
 *
 */
 ...
 *
 * S樹使用hist來決定是否執行平衡操作，hint類似AVL樹的高度屬性但不要求準確，hint提供最長鍊結
 * 的近似值。
 */
struct st_node {
    short hint;
 ...
 * inserted node.
 */
//於STree進行插點的動作遵循二元樹規則，插點完成後必須進行整個樹的更新
...
static void __treeint_dump(struct st_node *n, int depth)
{
    ...
    __treeint_dump(st_left(n), depth + 1);  // FFFF = st_left(n)

    struct treeint *v = treeint_entry(n);
    printf("%d\n", v->value);

    __treeint_dump(st_right(n), depth + 1);  // GGGG = st_right(n)
}
